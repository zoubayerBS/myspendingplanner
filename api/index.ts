...
// ── Auth (protected) ─────────────────────────────────
app.get('/api/auth/me', verifyTokenWithRevocation, async (req: any, res) => {
  try {
...
app.post('/api/auth/logout', verifyTokenWithRevocation, async (req: any, res) => {
  try {
...
app.patch('/api/users/:uuid/activate', verifyTokenWithRevocation, requireAdmin, async (req: any, res) => {
  try {
...
app.patch('/api/users/:uuid/role', verifyTokenWithRevocation, requireAdmin, async (req: any, res) => {
  try {
    const { uuid } = req.params;
    const { role } = req.body;
...
app.patch('/api/transactions/:id', verifyTokenWithRevocation, async (req: any, res) => {
  try {
    const { id } = req.params;
    const { type, amount, categoryId, date, note } = req.body;
    const validTypes = ['income', 'expense'];
